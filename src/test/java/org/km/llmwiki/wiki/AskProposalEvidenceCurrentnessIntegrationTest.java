...
class AskProposalEvidenceCurrentnessIntegrationTest extends IsolatedIntegrationTest {
    ...
    @Test
    void approveBlockedWhenSourceParseStatusIsNoLongerProcessed() throws Exception {
        seedWorkspaceWithSources();
        activate(lookupWorkspaceId("active"));
        mockMvc.perform(post("/api/v1/ask/proposals")
                        .contentType("application/json").content(requestBody()))
    ...
    @Test
    void approveBlockedWhenSourceChunkHashNoLongerMatchesNormalizedContent() throws Exception {
        seedWorkspaceWithSources();
        activate(lookupWorkspaceId("active"));
        mockMvc.perform(post("/api/v1/ask/proposals")
                        .contentType("application/json").content(requestBody()))
    ...
    @Test
    void approveBlockedWhenCanonicalWikiBytesDriftWithoutRevisionChange() throws Exception {
        seedWorkspaceWithSources();
        activate(lookupWorkspaceId("active"));
        mockMvc.perform(post("/api/v1/ask/proposals")
                        .contentType("application/json").content(requestBody()))
