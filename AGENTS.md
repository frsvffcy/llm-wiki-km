...
* Read-only local MCP adapter（#327）：`mcp.McpServerController`（`POST /api/mcp`）為 read-only-first、loopback-only 的 MCP Streamable HTTP 子集（JSON-RPC 2.0；initialize/ping/tools/list/tools/call）；MCP 是另一個 adapter 不是新 authority——tools 委派既有 application 契約的 controller boundaries（status/search/retrieval inspect/source locator/ask），無第二套 retrieval/ask pipeline，不直接操作 SQLite/FS/ArcadeDB/sqlite-vec/provider endpoint/key；auth token 為 backend-only configuration、fail-closed（unconfigured → deterministic `MCP_DISABLED`）、constant-time 比較、絕不進 response/log/manifest；body hard bound 在 decoded body enforcement；write tools deterministic `UNSUPPORTED_TOOL`；Ask 的 egress disclosure 重用 #323 config-level descriptors（labelled CONFIGURATION）＋ #310 execution-level `ProviderUsageStatus`（labelled EXECUTION；`NOT_ATTEMPTED` 不誤報）；無 write tools／remote bind／agent loop。
