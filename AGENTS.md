...
Criterion ↔ Evidence 詳細寫法見 `docs/development/criterion-evidence-traceability.md`（L3＋／correctness-sensitive 建議 stable `AC-01` IDs＋逐 AC evidence row；L1／L2 不強制完整矩陣）。
...
* **Browser first-mile（#352）**：vanilla-JS hash-navigation 多視圖骨架（Home／Inbox／Ask／Inspect／Review／Quality），Home/Inbox 為既有 Workspace／Inbox／Extraction REST 的 UI projection——workspace 建立/切換（切換後清空並重新取得 workspace-scoped state）、單檔/批次上傳（部分失敗如實呈現）、rescan、soft-delete、extraction 觸發與 bounded extracted-content preview、`DocumentStatus` typed 狀態徽章；無新增 authority、CSP 不放寬；processing job list endpoint 不存在（僅 per-jobId query），UI 不自造 job authority。
* **Browser Knowledge Quality triage（#383）**：`#/quality` 視圖為 Vault Lint findings 的唯讀 triage 工作區——`GET /api/v1/vault-lint/findings` 回傳 application-owned report projection（workspace-scoped、deterministic ordering、無 absolute path；`NoActiveWorkspaceException`→404 typed），Browser 提供 list／category／severity filter／detail（含 authoritative page preview 經既有 wiki read API、Wiki／Review hand-off links）；workspace 切換清空並重讀；finding 只 render、不重跑 lint、不解析 link target、不推導 repair eligibility（repair 屬 #384 authority）；零 mutation（無 POST／PATCH／PUT／DELETE；靜態斷言鎖定），無 innerHTML（純 textContent）。
* **Governed repair ingress（#384）**：deterministic health finding 經人類明確動作轉為受治理 Proposal 的唯一合法路徑——`POST /api/v1/repair/proposals` 只收 canonical `knowledgeId`，mutation 當下重驗 workspace／finding currentness／eligibility／lineage（無 finding→409 typed、ineligible→422 typed、foreign→404 不洩漏；plan 與 dedup hash 由當下 finding detail 重建，failure class 改變以新 plan 供完整 diff 審視）；eligibility 唯一 authority 為 backend（v1 僅 `CANONICAL_CONTENT_INVALID`＋resolvable governed lineage＋target 可讀的 restore 類可修，其餘 typed 拒絕；Unicode normalization 不在 eligible 之列）；repair proposal（`source_kind='REPAIR'`、MERGE、REVIEW 進入、dedup idempotency、REJECTED 不佔 index 可重新提案）走既有 Proposal → Draft → Human Review → explicit Publish 全流程（draft baseline pin 實際 drifted bytes 供完整 diff 檢視；publish 樂觀鎖維持）；Browser 只 render backend capability（double-submit guard、無 innerHTML、不傳 path／detail）；finding 不是 authorization token，不解鎖 MCP write／auto-fix／auto-publish。
* **Browser Wiki 消費工作區（#373）**：`/api/v1/wiki` read-only list/read API（workspace-scoped、PUBLISHED-only、canonical 內容經 content hash 驗證；`WikiPageNotFoundException`→404、reader validation/unavailable→409/503 typed），Browser `#/wiki` 視圖提供分頁/類型篩選清單與 markdown 純文字閱讀（含前往 Ask/檢視器 hand-off）；無 direct DB/FS、無 publish state 自行判斷。
* **Governed Ask → Proposal ingress（#374）**：Ask 維持 ephemeral/read-only（source-level boundary test 鎖定），`POST /api/v1/ask/proposals` 為**獨立的明確 mutation command**——citation 逐項驗證 current workspace state（unknown/stale/foreign 為 typed 422 `ASK_CITATION_INVALID` fail-closed；WIKI citation 以 path+ask-time revision 驗證 staleness）、per-workspace dedup hash（partial unique index）提供可測試 idempotency（重複提交回 200＋duplicate=true）、provenance 最小持久化（question/answer/citation identities＋provider model 實值，無 raw payload/secrets）；V30 rebuild 使 analysis-chain FK nullable＋`source_kind` 判別；Browser 僅於 grounded answer 後提供明確「轉為 Proposal」動作（double-submit guard、失敗可重試），建立後走既有 Proposal → Draft → Human Review → Publish authority。
* **Browser diagnostics workspace（#375）**：Inspect 視圖為 Retrieval Inspector＋Source Locator 的唯讀診斷工作區——Ask citation「檢視來源位置」開啟 locator 並導航至 `#/inspect`；answer 後「檢視檢索過程」以原問題 prefill Inspector（無 answer 不虛構查詢）；Wiki 閱讀面有 hand-off links；inspector/locator 結果為 current-workspace projection（workspace-changed 清空）；零 ranking/citation authority、零 side effect。
* **Vault Lint（#379）**：deterministic、bounded、read-only 的 canonical Wiki 健康掃描（application service，無 REST/CLI UI、無 auto-fix、無 LLM/provider egress）——broken internal link、orphan page（WARNING 觀察）、canonical content 驗證（重用 PublishedWikiContentReader authority，不建第二份 frontmatter schema）、dangling provenance（跨 workspace proposal reference）；finding 為 typed contract（code/category/severity/knowledgeId/logicalPath/bounded detail，deterministic ordering）；derived projection drift 刻意不掃（歸 #116/#286 authority）。
* **Browser inline citation preview（#381）**：Ask citation「檢視來源位置」改為 **inline** 於 answer 視圖渲染 authoritative locator（重用 diagnostics workspace 同一 renderer；不再自動跳轉）——metadata 依 locator 實際精度漸進呈現（document/chunk/page/section/heading，缺席欄位不顯示、不偽造 bbox/offset highlight）；bounded preview 以安全 DOM highlight 區塊呈現（textContent，零 innerHTML）；not-current 由 backend contract 拒絕暴露內容；stale/not-found/unavailable 為 inline typed states；diagnostics workspace 仍可由 hand-off links 進入。
* **Browser review workbench（#353）**：Review 視圖為 Proposal → Draft → Human Review → Publish 的三段式治理工作台——Proposal list/filter/detail/status transition（轉換按鈕由 backend `allowedTransitions` capability projection 驅動、單一 domain authority 推導、fail-closed，核准不自動 publish，#370）、Draft create/preview/diff/regenerate/invalidate（get/preview/diff 的隱式失效由 backend 重驗並如實渲染）、publish 為明確人類動作（double-submit guard＋typed `WikiPublishResult`/outcome＋失敗清除先前成功顯示＋發布後重讀 authoritative state）；workspace 切換清空全部治理 state；無 innerHTML（純 textContent）；Ask 仍為 ephemeral、無 Save Answer to Knowledge。
* **Owner security 與 deployment（#417／#418／#422／#423）**：single-user owner boundary（預設關閉；in-memory session＋Host／Origin／throttling；credential 為 versioned salted adaptive KDF，legacy hash 僅 LOCAL_ONLY migration aid）＋ single-instance deployment profile（raw backend 永遠 loopback-only；Mode 0 `LOCAL_ONLY` SUPPORTED／CURRENT；Mode 1 `PRIVATE_INGRESS` 需 canonical browser origin 四方對齊才 SUPPORTED 否則 `NOT_READY`；Mode 2 public HTTPS 永 CANDIDATE；direct bind REJECT）；auth 只做 admission，MCP 維持 loopback read-only。
...
* Structure-preserving ingestion：`ParsedDocument` typed blocks + versioned `ChunkingPolicy`（production default `chunk-policy-v1-current`；`source_chunk.chunk_policy_version`，policy 變更需重新 extraction）+ versioned `NormalizationPolicy`（#412：`normalization-policy-v1-current` rollback target／`normalization-policy-v2-selected-cf-strip` production default，僅 U+00AD/U+200B/U+2060/非開頭 U+FEFF，ZWJ/ZWNJ/BiDi 永不 strip；`source_chunk`/`document_extracted_content.normalization_policy_version` lineage，upgrade/rollback 皆需 explicit re-extraction）。
...
* Query transformation recall evaluation（#390）：query-side semantic transformation（rewriting／multi-query／HyDE）採 benchmark-first evaluation-only——`QueryTransformationScopeBoundaryTest`（source-level）證明 production 唯一 query 側轉換是 #129 `cjk-bigram-v1` deterministic lexical projection（AskService stage 順序 retrieve(question 原樣)→rerank→projection，無 semantic rewrite boundary）；`query-transformation-evaluation-corpus-v1`（逐字重用 #316 corpus＋multi-intent/no-evidence shapes）在 production-equivalent stack 上量測 ORIGINAL_QUERY vs SINGLE_REWRITE（typed rewrite fixture：protected 候選＋unprotected probe，per-rewrite token-overlap audit 對照 candidate-bias），candidate-generation pool recall（per-channel，經 Retrieval Inspector collector）與 fused-window recall@8/MRR 分開量測，typed miss taxonomy（authority/crowd-out/graph-only/wording-mismatch/backend-unavailable）零誤歸因，blocking gates 含 forbidden identity、EXACT_TOKEN/graph-added retention、original query 恆為第一個 retrieval input、fan-out ≤3、四種 typed fallback 與 baseline 逐 identity 一致、兩次 pass 可重現；verdict **CONDITIONAL GO**——pool 無增益（fixture vector tie 飽和）但 `property-token` 的 wording-mismatch crowd-out 經 protected rewrite lexical 命中恢復 window recall（0→1.0），unprotected probe 證實 exact-token protection 為 adoption 硬性要件；production adoption 須另開 issue（versioned policy、typed applicability/no-op、exact-token protection、hard fan-out、#323/#310 egress disclosure、live-provider controlled measurement、provider-free fusion-side levers 的 cost 比較）；本 evaluation 無 production 變更、無新 public retrieval mode、無 Browser 變更。
...
* **Automation mutation ownership**：agent／hook／formatter／generator 只能修改或 stage 本次任務明確擁有的 change set；pre-existing dirty tracked、untracked 與 local-only state 預設唯讀，不得修改、stage 或刪除。重新產生產物須由任務明確觸發，先檢查、再 review diff，並以明確 path stage；automation 不得無聲擴張 staged set。完整契約與 current surface 盤點見 `docs/development/action-risk-autonomy.md` §8。
* Branch 從最新 `main` 建立（舊 branch merge 後不得續用）；命名 `feature|fix|test|cleanup/<issue>-<slug>`。
* PR：target `main`（stacked PR 須標示 parent 與進 main 路徑 + `PR-Metadata-Exception: stacked-pr`；非 issue-driven 加 `PR-Metadata-Exception: non-issue-driven`）；標題/說明繁體中文；body 至少含摘要、相關 Issue（逐一 non-closing reference `Refs #N`，禁止 `Closes/Fixes/Resolves #`；`PR #N` 等 PR lineage 可另行註記但不充當 Issue linkage，`Refs #N` 指向 PR 仍 fail closed，#411）、主要變更、AC、驗證方式/結果（如實記錄，不得虛構或省略已知失敗）。PR Metadata 以當下 GitHub PR metadata 為 authority（rerun 不重用 stale event body，取回失敗 fail-closed）。
