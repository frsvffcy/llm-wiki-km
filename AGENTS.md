...
## 0. 語言與溝通規範
* **人類可讀內容**：Git Commit 說明、PR 標題與說明、GitHub Issue 標題與內容、Code Review comment、開發文件與代理進度回報，原則上使用繁體中文（臺灣用語）。
...
* **套件管理**：
  ...
  * **版本控管**：Git
* **階段邊界與防護欄（Phase Gate）**：
  * **Phase 1 (v0.1 ~ v1.0 / Sprint 0~7)**：Foundation、Inbox/Archive、Tika Extraction、Job Engine、LLM Proposal/Review、Wiki Publish (Obsidian 相容 Markdown)、SQLite FTS5 全文搜尋。
  * **Phase 2 (v1.2 ~ v1.5 / Sprint 8~11)**：Knowledge Graph-lite (SQLite)、Embedding + sqlite-vec / Vector Search、Hybrid RAG。
  * **Phase 3 (v1.8 ~ v2.0 / Sprint 12~14)**：Neo4j Projection、GraphRAG。
  * **禁止越級原則**：在目前 Sprint/Issue 未進入相應 Milestone 前，**嚴禁提前引入** sqlite-vec、Neo4j、GraphRAG 等依賴與實作。

## 2. 核心執行指令 (Build & Test Commands)
> 測試選擇採風險導向：coding loop 預設 targeted-test-first；PR Ready 的 safety gate 仍是 `mvn clean verify -Pfull` 與 `git diff --check`。`-DskipTests` 只能作 preliminary verification，不得作為 final gate。
* **初步依賴／建置確認（非 final）**：
  ```bash
...
* **PR Ready / Final**：
  ...
  git diff --check
  ```
  `-Pfull` 會依目前 `pom.xml` 的 generate-sources lifecycle 重新產生 jOOQ sources，並執行完整 regression、clean build integrity 與 package/verification；不需在 AGENTS 中維護另一套手動 codegen 流程。`mvn clean package` 可作中途 package smoke check，但不得取代此 final gate。
...
* **目錄結構**：
  ...
  * Package 分層必須遵循既有骨架與設計文件的模組規劃：
    ...
    ├── search/        # Metadata 搜尋、SQLite FTS5 全文搜尋、(未來) Vector/Hybrid
    ├── rag/           # (未來 Phase 2) Retrieval、Rerank、Context、Answer
    ├── graph/         # (未來 Phase 2/3) Entity、Relation、Graph Sync、Neo4j
...
* **不確定時暫停**：涉及破壞性修改或架構大改時，必須先產出 Markdown 企劃書，等待人類確認後再動手。
  * 包括但不限於：修改已套用的 Flyway migration、變更 `/api/v1` 既有契約、調整 Document ↔ Wiki 資料模型、提前導入 Vector/Neo4j（超出 MVP 版本範圍）。
...
* **分支命名**：使用英文小寫 slug，不使用中文、空白或特殊字元。
  * 功能：`feature/<issue-or-story>-<簡短英文描述>`，例如 `feature/story-201-document-parser`
  * Bug 修正：`fix/<issue>-<簡短英文描述>`，例如 `fix/issue-60-transactional-inbox-delete`
...
* 可適度引用 Issue，例如：`fix: 修正 Inbox delete 交易完成前提前清除 staged file（#60）`。
...
* 新增 persistent application table 時，必須檢查 `testsupport.IsolatedIntegrationTest` 的 reset strategy 是否涵蓋該 table。
...
* 修單一 failed test 時，先重跑該 class 或 affected suite；不可因單一失敗立即重跑 300+ tests。完成 affected diagnosis 後，再依 scope 決定 Feature Ready 與 Final Verification。
...
* **Final / PR Ready**：目前要求 `mvn clean verify -Pfull` 及 `git diff --check`；PR targeting `main` 還必須等待 PR CI 的 full job 成功。PR Ready 不得只靠 fast tests。
...
* shared context、SQLite cleanup 與 reset strategy 必須維持測試隔離；新增 persistent table 時同步檢查 `testsupport.IsolatedIntegrationTest` 的 reset hook。#139 的 cleanup completeness guard 尚未合併前，不得宣稱 guard 已存在，仍須人工核對。
...
* Canonical ownership 以 `docs/development/testing.md` 的最新 tracked 定義為準。至少要追蹤下列跨 Story invariant 的 suite-scope owner：workspace isolation → workspace/filesystem integration；FTS serving freshness／projection version → Search/FTS serving consistency；Retrieval failure semantics → Retrieval contract/integration；FTS rebuild／health／restart recovery → FTS rebuild-health integration；CJK search quality → CJK search-quality contract。Exact class owner 應以 testing docs 的實際定義為準。
* 截至本規範建立時，#139 仍為 OPEN，ownership 文件與 guard 尚未視為已存在；因此本節不預先硬編不存在的 class 名稱。#139 完成後，應以 testing docs 的實際 suite owner 更新本節與相關 Issue/PR evidence。
...
* #139 的 automated tag coverage guard 尚未合併前，工程師仍須在 test architecture 變更時手動核對 test inventory；guard 合併後，AGENTS 要求該 guard 通過，並在 PR body 記錄 inventory/count。
