...
* **Governed repair ingress（#384）**：deterministic health finding 經人類明確動作轉為受治理 Proposal 的唯一合法路徑——`POST /api/v1/repair/proposals` 只收 canonical `knowledgeId`，mutation 當下重驗 workspace／finding currentness／eligibility／lineage（stale→409 typed、ineligible→422 typed、foreign→404 不洩漏）；eligibility 唯一 authority 為 backend（v1 僅 `CANONICAL_CONTENT_INVALID`＋resolvable governed lineage 的 restore 類可修，其餘 typed 拒絕；Unicode normalization 不在 eligible 之列）；repair proposal（`source_kind='REPAIR'`、MERGE、REVIEW 進入、dedup idempotency）走既有 Proposal → Draft → Human Review → explicit Publish 全流程（draft baseline pin 實際 drifted bytes 供完整 diff 檢視；publish 樂觀鎖維持）；Browser 只 render backend capability（double-submit guard、無 innerHTML、不傳 path／detail）；finding 不是 authorization token，不解鎖 MCP write／auto-fix／auto-publish。
