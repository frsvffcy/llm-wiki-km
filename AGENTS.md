...
* **Browser review workbench（#353）**：Review 視圖為 Proposal → Draft → Human Review → Publish 的三段式治理工作台——Proposal list/filter/detail/status transition（僅 mirror backend state machine，核准不自動 publish）、Draft create/preview/diff/regenerate/invalidate（get/preview/diff 的隱式失效由 backend 重驗並如實渲染）、publish 為明確人類動作（double-submit guard＋typed `WikiPublishResult`/outcome＋失敗清除先前成功顯示＋發布後重讀 authoritative state）；workspace 切換清空全部治理 state；無 innerHTML（純 textContent）；Ask 仍為 ephemeral、無 Save Answer to Knowledge。
