...
* Read-only local MCP adapter（#327／#330）：`mcp.McpServerController`（`POST /api/mcp`）為 read-only-first、loopback-only 的 MCP Streamable HTTP adapter；current `2026-07-28` 採 stateless `server/discover`/ping/tools list/call（每次 request 驗 version/client metadata、`Mcp-Method`、需要時的 `Mcp-Name`），bounded legacy `2025-06-18` 才允許 initialize flow，兩個 era 不共享 hidden state。處理順序固定 Host/Origin exact loopback guard → backend-only constant-time bearer auth/fail-closed `MCP_DISABLED` → media semantics → decoded-body hard bound → protocol validation → dispatch；GET/DELETE 明確 405，modern 不接受 client notification。MCP 是另一個 adapter 不是新 authority——五個唯讀 tools 委派既有 status/search/retrieval inspect/source locator/ask application contracts，無第二套 retrieval/ask pipeline、不直接操作 SQLite/FS/ArcadeDB/sqlite-vec/provider endpoint/key；write tools deterministic `UNSUPPORTED_TOOL`；Ask egress disclosure重用 #323 CONFIGURATION＋#310 EXECUTION typed metadata；無 write tools／remote bind／agent loop。Codec adoption decision 為 `KEEP_CUSTOM_CODEC`，evidence與重評門檻見 `docs/development/issue-330-mcp-transport-compatibility.md`。
