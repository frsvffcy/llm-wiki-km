...
* **階段邊界與防護欄（Phase Gate）**：
  ...
  * **Phase 3B / completed / CONDITIONAL GO**：embedded multi-model feasibility spike 已由 Issue #240 與 [ADR 0008](docs/adr/0008-arcadedb-embedded-projection-feasibility-spike.md) 完成。ArcadeDB 是目前首選的 production projection adapter candidate，僅可承接可重建的 Document / Vector / Graph / Search projection；spike 使用的 exact dependency version 只屬可重現證據，不是永久 architecture pin。這不代表 production runtime 已採用 ArcadeDB，也不授權 Graph Retrieval 或 GraphRAG。
  * **Phase 3 production-adoption gate / current next step**：下一張候選 Story 為 `[L5][Sprint 8][STORY-803] 建立 production ArcadeDB Graph projection adapter 與 lifecycle/readiness/repair adoption gate`。其責任至少包括 production adapter wiring/configuration、provider-neutral `GraphProjectionWriter` / `GraphProjectionRebuilder` implementation、SQLite-backed control-plane lifecycle/readiness authority、rebuild/repair/health/diagnostics、process restart/crash recovery、concurrency/file-locking/multi-process limitation evidence、backup/delete/rebuild operational policy、dependency/license/security review、Linux CI 與 Apple Silicon/local evidence，以及 failure/degradation semantics。Graph backend unavailable 時必須維持 lexical + vector baseline。
  * **Phase 3C / not yet authorized**：bounded Graph Retrieval、candidate revalidation 與 `EvidenceBundle` integration 僅能在 production projection lifecycle 被證明後開始；GraphRAG 仍是後續能力。SQLite 持續作為 operational/control plane，負責 relational schema、SQLite FTS5、readiness 與 authority/provenance enforcement；不得將其 migration 或替換。ArcadeDB 不得成為 SQLite replacement、canonical Source of Truth 或 domain authority。domain/application 不得依賴 ArcadeDB API、record model、vendor DTO、Cypher、GQL 或 SQL-PGQ；一律經 provider-neutral adapter boundary。Neo4j、RyuGraph、BigQuery Graph 與 Spanner Graph 皆保留為未來 adapter candidates，必須通過 adoption gate，且不是唯一 backend 或近期必要 dependency。Graph candidate 必須經 workspace-scoped authority、provenance、freshness 與 eligibility revalidation，才可進入 `EvidenceBundle`，並沿用既有 citation 與 grounded Answer contract。Graph unavailable 時維持 lexical + vector baseline；Vector/backend unavailable 時維持既有 typed diagnostics 的 lexical degraded fallback，不得以 stale 或未重新驗證的 candidate 補位。詳細 contract、bounded traversal 與 adoption gate 見 [ADR 0007](docs/adr/0007-provider-neutral-knowledge-graph-and-graph-retrieval.md)。
