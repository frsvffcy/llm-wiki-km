...
| `2026-09-19-graphify-cross-artifact-code-intelligence-evaluation.md` | `TRACK_FULL` | #535 Phase B G0／G1 已完成；Graphify 為 materially-different candidate，但 correctness blocking gaps 未解除，final status 為 `DEFER / BENCHMARK DONE`；production／Product Graph integration 維持 NO-GO | **cross-artifact graph（code + docs/ADR + SQL + Bash/YAML/manifests）**、edge provenance `EXTRACTED/INFERRED/AMBIGUOUS`、built-at-commit／health／fail-closed derived-index治理、`useful/dead_end/corrected` outcome feedback sidecar | 全文納入；R2／R3／R5／R9 有 materially-different recovery，G1 找回 R7，但 R8／R6／R10 僅 partial、R4 仍 miss；不採 default／owner-optional／graph-first／strict-first／remote MCP／product integration |
...
| `2026-09-19-onyx-enterprise-rag-evaluation.md` | `TRACK_FULL` | #545 將 Onyx 收斂為 enterprise RAG control-plane / benchmark design input，不導入 runtime、Vespa/Redis/MinIO、connector或 multi-tenant stack；#546 承接 query-shape coverage gap | **Search Receipt = retrieval metadata not evidence**、scope-unrepresentable即不出 receipt、connector capability typed preflight、lowest-layer tenant/workspace authority、EnterpriseRAG-Bench conflict/completeness taxonomy | 全文納入；Search Receipt併回 #533/#541；connector capability維持 trigger-gated DEFER；唯一新 owner #546，不建立 Onyx-specific roadmap |
...
- `MATERIAL DIFFERENCE — MEASURED`：Phase B G0 找回 R2／R3／R5／R9 的不同 discovery signal；G1 用 pinned prompt＋host-agent fallback 找回 R7，並對 R8／R6／R10 提供 partial signal。`claude-cli` 自動 backend 因 `Not logged in` 未執行；未新增、也不需要新增 API key。
- `CORRECTNESS CEILING — CONFIRMED`：R4 Flyway 仍 miss；JS↔REST URL contract、Flyway semantic coupling、shell→Maven→Java 與 Spring runtime wiring 仍是 blocking gaps。Semantic edges只能作 discovery、不可作 proof；empty／no-path 不可證明 dependency不存在。
...
- `DEFER / BENCHMARK DONE — #535`：G0／G1 已足額完成 bounded replay；blocking rule 未解除，因此不升格 default 或 owner-optional sidecar，不採 graph-first／strict-first／remote MCP／Product integration。Workflow-level efficiency未做 matched end-to-end measurement，不宣稱 MEASURED gain（沿 #509）。
- 不建立 vendor-specific permanent roadmap；Graphify decision 已回寫同一 developer code-intelligence sidecar family。
...
Onyx 的 current lineage 是 **enterprise RAG control-plane / search observability / connector capability / benchmark-coverage design input**，不是新的 Product RAG 或 enterprise-platform roadmap：

- `SOURCE PIN`：`onyx-dot-app/onyx` default `main` audited `a239d812...`；latest release `v4.7.7`（2026-09-16）；Community/core非 `ee/` 為 MIT，`ee/` 目錄使用 Onyx Enterprise License。Related benchmark `EnterpriseRAG-Bench` audited `d36685e2...`。
- `CURRENTLY COVERED`：hybrid keyword/vector、query expansion、weighted RRF、rerank、context expansion、citation、workspace/scope、MCP、operations/observability等，本專案已有 current owner；不得建立第二套 Onyx SearchTool、Vespa/Redis/MinIO、multi-tenant/RBAC 或 connector runtime。
- `SEARCH RECEIPT`：Onyx將 executed query lanes、new/repeated document counts、post-fusion cap、final evidence count與 scope交給後續 reasoning，但明確標為 **retrieval metadata, not source evidence**；empty/repeated results不證明資訊不存在。若 active retrieval scope無法完整表示，receipt直接不發出（`unrepresentable_scope`），這是 #541 environment/scope honesty與 #533 adaptive evidence-gap retrieval的 high-value input。
- `EXPERIMENT CALIBRATION`：Search Receipt current 107-pair A/B為 promising signal，但 paired bootstrap 95% interval跨 0，mechanism亦未被證明，Onyx自身維持 experimental；不得把 headline gain當 llm-wiki-km ROI，只借鏡 paired design、outlier/timeout/cost/latency/uncertainty disclosure。
- `CONNECTOR CAPABILITY`：credential preflight把 INDEXING／DOC_PERMISSION_SYNC／EXTERNAL_GROUP_SYNC 等 capability拆開，區分 PASSED／FAILED／INDETERMINATE／SKIPPED／NOT_APPLICABLE／PASSED_WITH_WARNINGS；required check才 gate capability，timeout/transient≠broken，config hash決定 report currentness。Current llm-wiki-km尚無 external connector runtime，因此 `DEFER / FUTURE CONNECTOR INPUT`，第一個 credentialed source connector立項時再啟動。
- `LOWEST-LAYER SCOPE AUTHORITY`：current Onyx #14827修正 index scan，storage/index-owned tenant state必須覆寫 caller-supplied tenant filter，ACL再疊加；對本專案是 current workspace/provenance/revalidation invariant的 reinforcement，不是新 bug。
- `PERMISSION PROJECTION`：Onyx scoped-permission設計用同一 `can_<action>` helper同時支撐 server enforcement、DTO permission projection與 contract test，形成 `project == enforce by construction`；本專案 current single-owner故 `DEFER`，future multi-user/RBAC才重評。
- `BENCHMARK COVERAGE — #546`：EnterpriseRAG-Bench提供 Basic／Semantic／Intra-Document／Project／Constrained／Conflicting Info／Completeness／High Level／Info Not Found taxonomy。Fresh audit顯示 current corpora已有 lexical/semantic/graph/multi-relevant/multi-intent/no-evidence/stale/foreign，但缺明確 answer-level `CONFLICTING_INFO` 與 `COMPLETENESS_REQUIRED` truth，因此 #546只補 vendor-neutral query-shape coverage matrix與最小 deterministic golden cases。
...
| RAG query-shape coverage / evidence completeness truth | EnterpriseRAG-Bench（#545）、VikingRAG sufficiency（#533）、gold-span truth（#531）、current Graph/Rerank/Query Transformation corpora | `ACTIONABLE → #546`；保留既有 NO_EVIDENCE，不重複造 case；新增最小 CONFLICTING_INFO 與 COMPLETENESS_REQUIRED deterministic gold，INTRA_DOCUMENT_DISTANT視 current chunk truth可行性決定。此 row回答「題型有沒有覆蓋 failure shape」，#541則回答「實驗有沒有真的測到 feature」 |
| Future external-source connector capability | Onyx credential capability checks（#545）、OpenWiki connector isolation（#443）、current provider-egress governance（#323） | `DEFER / TRIGGER-GATED`；第一個 credentialed remote source connector立項時才定義 INDEX/SYNC/ACL/METADATA 等 capability與 PASSED/FAILED/INDETERMINATE/SKIPPED/NOT_APPLICABLE/PASSED_WITH_WARNINGS semantics；timeout/transient不得誤判 credential broken，report需綁 config currentness |
