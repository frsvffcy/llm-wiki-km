...
- `CONDITIONAL GO — BENCHMARK`：#505 以 pinned v2.3.8、local-only、read-only、無 cloud embedding 的 10 個 historical L3/L4/L5 replay case，和 current grep/git/source-read baseline 做 correctness-first A/B（2026-09-18/19 已執行，見該 evaluation §12）；結果觸發 blocking rule（跨層／MockMvc tests／Flyway／CI 全系性 miss），**不晉升 Stage 2**，轉 `DEFER`；
