...
| Developer orchestration / historical replay calibration | Dream-RSI（#519）、#505/#509 historical replay evidence discipline、current model-routing §7 | `CONDITIONAL GO — #520`；只在 developer evaluation plane比較 routing／escalation／challenge policy。完整 Issue/PR trace作 split unit，correctness hard gate先於 cost，replay winner需 fresh unseen canary；不自動改治理、不建立 Product runtime或 vendor-specific RSI roadmap |
