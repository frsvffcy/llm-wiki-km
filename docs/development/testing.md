...
Hard gates（任何一項違反即 fail）：safety violations 恆為空（stale hash、外部 workspace、`MENTIONS`-only 三類負向樣本在任何 mode 都不得被 retrieve）；baseline modes 的 graph-only found 恆為 0；`HYBRID_GRAPH` 必須找回 graph-only relevant target（graph-added 增益不是 fixture artifact）；rebuild 前 `HYBRID_GRAPH` 對 NOT_READY projection 必須 typed degradation 且保留 lexical+vector baseline（`degradedBaselineRetained`）。Metric floors 為量測 baseline 減 headroom：目前量測值 HYBRID_FTS 0.444/0.667、HYBRID_VECTOR 0.889/1.000、HYBRID_GRAPH 1.000/0.833（recall/MRR），floors 0.40/0.60、0.80/0.90、0.95/0.75；floors 是 floor 不是 pin，低於 floor 的下降才視為 regression，更新 corpus 或 ranking 後必須同步檢查 baseline 記錄。此 suite 的 report 是文字資產、不進 Git；重跑命令：`mvn test -Dtest=GraphRetrievalQualityGateTest -Pintegration`。
