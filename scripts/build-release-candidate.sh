...
export M_PROJECT="$ARTIFACT_ID" M_VERSION="$PROJECT_VERSION" M_COMMIT="$SOURCE_COMMIT"
...
# Versioned notes are derived from Maven (no second version truth): historical
# notes stay immutable for their tags; each candidate bundles its own versioned notes.
NOTES_SOURCE="docs/release/v${PROJECT_VERSION}-release-notes.md"
[ -f "$NOTES_SOURCE" ] || fail "versioned release notes missing: $NOTES_SOURCE (keep historical notes immutable, add v${PROJECT_VERSION})"
