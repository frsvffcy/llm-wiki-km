...
#
# Candidate identity (Refs #458): the exact filename is derived from the
# Maven authority (project.artifactId + project.version) and resolved via
# release_identity_resolve_candidate_jar — never by mtime recency.
# An explicit --jar must name that same file; the JAR-internal versions are
# verified and, when the release sidecar manifest exists, it must describe
# this exact JAR before the smoke body runs.
set -eu
...
while [ $# -gt 0 ]; do
  case "$1" in
    --jar) JAR="${2:-}"; [ -n "$JAR" ] || { echo "[backup-restore-smoke] FAIL: --jar requires a path" >&2; exit 2; }; shift 2 ;;
...
echo "[backup-restore-smoke] candidate: $JAR"
